...
<div class="paiement_block fh-stack">
    ...
    {block name='order_payment_classic_terms_and_conditions'}
        <div id="tc_cont" class="box fh-check-cont">
            <p class="checkbox fh-check">
                ...
                <a id="tc_link" href="{$link_conditions|escape:'html':'UTF-8'}" class="iframe fh-check-cont__link" rel="nofollow" >{l s='(Read the Terms of Service)'}</a>
            </p>
        </div>
    {/block}
    ...
    <div class="fh-oc-card">
        <div class="fh-oc-card__head">
            <p class="block-small-header fh-eyebrow">{l s='PAYMENT RESOURCE'}</p>
        </div>
            ...
            {if $HOOK_PAYMENT}
                {if !$opc}
                    {block name='order_payment_classic_cart_summary'}
                        <div id="order-detail-content" class="table_block table-responsive fh-card">
                            <p class="fh-eyebrow">{l s='Order summary'}</p>
                            <table id="cart_summary" class="table table-bordered">
                                ...
                                {if $total_shipping_tax_exc <= 0 && (!isset($isVirtualCart) || !$isVirtualCart) && $free_ship}
                                    <tr class="cart_total_delivery">
                                        ...
                                        <td colspan="2" class="price" id="total_shipping"><span class="fh-badge">{l s='Free Shipping!'}</span></td>
                                    </tr>
                                {else}
                                ...
                                {if $use_taxes}
                                    ...
                                    {/if}
                                    <tr class="cart_total_price fh-cart-total__row--total">
                                            ...
                                            <td colspan="2" id="cart_voucher" class="cart_voucher">
                                                <div id="cart_voucher" class="table_block">
                                                    {if isset($errors_discount) && $errors_discount}
                                                        <ul class="alert alert-danger fh-alert fh-alert--error">
                                                    ...
                                                    {if $voucherAllowed}
                                                        <form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
                                                            <fieldset>
                                                                <h4 class="fh-oc-voucher-head">{l s='Vouchers'}</h4>
                                                                ...
                                                                <button type="submit" name="submitAddDiscount" class="button btn btn-default button-small fh-btn fh-btn--sm fh-btn--primary"><span>{l s='ok'}</span></button>
                                                                {if $displayVouchers}
                                                                    <p id="title" class="title_offers fh-oc-voucher-sub">{l s='Take advantage of our offers:'}</p>
                ...
                {/if}
            {else}
                <p class="alert alert-warning fh-alert fh-alert--warn">{l s='No payment modules have been installed.'}</p>
            {/if}
        </div>
    </div>
    {if !$opc}
    <p class="cart_navigation clearfix">
        <a href="{$link->getPageLink('order', true, NULL, "step=2")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="button-exclusive btn btn-default fh-btn fh-btn--ghost">
