...
<div class="order-detail-content">
    {if isset($cart_htl_data) && $cart_htl_data}
        {block name='shopping_cart_heading'}
            <p class="cart_section_title fh-eyebrow">{l s='rooms information'}</p>
        {/block}
        {foreach from=$cart_htl_data key=data_k item=data_v}
            {foreach from=$data_v['date_diff'] key=rm_k item=rm_v}
                <div class="row cart_product_line fh-card">
                        ...
                        {block name='shopping_cart_room_type_cover_image'}
                            <p>
                                <a href="{$link->getProductLink($data_v['id_product'])}">
                                    <img src="{$data_v['cover_img']}" class="img-responsive img-room-type" />
                                </a>
                            </p>
                            <p class="room_remove_block">
                                <a class="cart_room_delete fh-btn fh-btn--ghost fh-btn--sm" href="{$rm_v['link']}" data-id_product="{$data_v['id_product']}" data-date_from="{$rm_v['data_form']}" data-date_to="{$rm_v['data_to']}" data-qty="{$rm_v['num_rm']}"><i class="icon-trash"></i> &nbsp;{l s='Remove'}</a>
                            </p>
                    ...
                    <div class="col-sm-10">
                            ...
                            {block name='shopping_cart_room_detail'}
                                <div class="product-xs-info">
                                    {block name='shopping_cart_room_type_name'}
                                        <p class="product-name fh-card__title">
                                            <a href="{$link->getProductLink($data_v['id_product'])}">
                                                {$data_v['name']}
                                            </a>
                                            <a class="btn btn-default pull-right product-xs-remove fh-btn fh-btn--sm fh-btn--ghost" href="{$rm_v['link']}"><i class="icon-trash"></i></a>
                                    ...
                                    {block name='shopping_cart_room_type_hotel_location'}
                                        {if isset($data_v['hotel_info']['location'])}
                                            <p class="hotel-location fh-text-muted">
                        ...
                        {block name='shopping_cart_room_type_features'}
                            {if isset($data_v['hotel_info']['room_features'])}
                                <div class="room-type-features fh-chip-row">
                                {foreach $data_v['hotel_info']['room_features'] as $feature}
                                    <span class="room-type-feature fh-chip">
                                ...
                                {block name='shopping_cart_room_type_and_service_price'}
                                    <div class="col-sm-7 margin-btm-sm-10">
                                        {if $rm_v['amount'] && isset($rm_v['total_price_without_discount']) && $rm_v['total_price_without_discount'] > $rm_v['amount']}
                                            <span class="room_type_old_price fh-text-muted">
                                                ...
                                                <div class="price_block">
                                                    <p class="total_price">
                                                        ...
                                                        {if (($rm_v['amount'] - $rm_v['amount_without_auto_add']) > 0) && (in_array($data_v['id_product'], $discounted_products) || $PS_ROOM_PRICE_AUTO_ADD_BREAKDOWN)}
                                                            ...
                                                            <div class="room-price-detail-container fh-card" style="display: none;">
                                                    ...
                                                    <p class="total_price_detial fh-text-muted">
                                                ...
                                                <div class="col-xs-6">
                                                    <div class="demand_price_block">
                                                        ...
                                                        <p class="total_price_detial">
                                                            <a data-date_from="{$rm_v['data_form']|escape:'html':'UTF-8'}" data-date_to="{$rm_v['data_to']|escape:'html':'UTF-8'}" data-id_product="{$data_v['id_product']|escape:'html':'UTF-8'}" data-action="{$link->getPageLink('order-opc')}" class="open_rooms_extra_services_panel fh-btn fh-btn--sm fh-btn--ghost" href="#rooms_type_extra_services_form">
                                                                {l s='Extra Services'}
                                                            </a>
                                                        </p>
                                                    </div>
    ...
    {if (isset($hotel_products) && $hotel_products) || (isset($standalone_products) && $standalone_products)}
        <p class="cart_section_title fh-eyebrow">{l s='Product information'}</p>
    {/if}
                ...
                <div class="col-sm-2 product-img-block">
                    <p>
                        <a href="{$link->getProductLink($product['id_product'])}">
                            <img src="{$product['cover_img']}" class="img-responsive img-room-type" />
                        </a>
                    </p>

                    <p class="product_remove_block">
                        <a id="{$product.id_product}_{if $product.id_product_option}{$product.id_product_option}{else}0{/if}_{if $product.id_hotel}{$product.id_hotel}{else}0{/if}" class="cart_quantity_delete fh-btn fh-btn--ghost fh-btn--sm" href="{$link->getPageLink('cart', true, NULL, "delete=1&amp;id_product={$product.id_product|intval}&amp;token={$token_cart}")|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='Delete'}">
                    ...
                    <div class="product-info-container">
                        ...
                        <div class="product-xs-info">
                            <p class="product-name fh-card__title">
                                ...
                                <a class="btn btn-default pull-right product-xs-remove fh-btn fh-btn--sm fh-btn--ghost" href="{$link->getPageLink('cart', true, NULL, "delete=1&amp;id_product={$product.id_product|intval}&amp;ipa=0&amp;id_address_delivery={$product.id_address_delivery}&amp;token={$token_cart}")|escape:'html':'UTF-8'}"><i class="icon-trash"></i></a>
        ...
        {foreach from=$standalone_products key=data_k item=product}
            <div class="row cart_product_line fh-card">
                <div class="col-sm-2 product-img-block">
                    ...
                    <p class="product_remove_block">
                        <a id="{$product.id_product}_{if $product.id_product_option}{$product.id_product_option}{else}0{/if}_0" class="cart_quantity_delete fh-btn fh-btn--ghost fh-btn--sm" href="{$link->getPageLink('cart', true, NULL, "delete=1&amp;id_product={$product.id_product|intval}&amp;token={$token_cart}")|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='Delete'}">
                            <i class="icon-trash"></i> &nbsp;{l s='Remove'}
                        </a>
                        {* <a href="{$rm_v['link']}"><i class="icon-trash"></i> &nbsp;{l s='Remove'}</a> *}
                    </p>
                    ...
                    <div class="product-info-container">
                        ...
                        <div class="product-xs-info">
                            <p class="product-name fh-card__title">
                                ...
                                <a class="btn btn-default pull-right product-xs-remove fh-btn fh-btn--sm fh-btn--ghost" href="{$link->getPageLink('cart', true, NULL, "delete=1&amp;id_product={$product.id_product|intval}&amp;token={$token_cart}")|escape:'html':'UTF-8'}"><i class="icon-trash"></i></a>
                                {block name='displayCartProductNameAfter'}
                                    {hook h='displayCartProductNameAfter' id_product=$product['id_product']}
                                {/block}
                            </p>
                                ...
                                <div class="col-xs-5">
                                    <div class="quantity_cont">
                                        ...
                                        <div class="cart_quantity_button">
                                            ...
                                            {else}
                                                <a class="cart_quantity_down btn btn-default disabled fh-btn fh-btn--sm fh-btn--ghost" href="#" id="cart_quantity_down_{$product.id_product}_{if $product.id_product_option}{$product.id_product_option}{else}0{/if}_0" title="{l s='You must purchase a minimum of %d of this product.' sprintf=$product.minimal_quantity}">
                                                    <span><i class="icon-minus"></i></span>
                                                </a>
                                            {/if}
        ...
        {block name='shopping_cart_proceed_action'}
            <div class="row">
                <div class="col-sm-12 proceed_btn_block">
                    <a class="btn btn-default button button-medium pull-right fh-btn fh-btn--primary" href="{$link->getPageLink('order-opc', null, null, ['proceed_to_customer_dtl' => 1])}" title="Proceed to checkout" rel="nofollow">
                        <span>
                            {l s='Proceed'}
                        </span>
                    </a>
