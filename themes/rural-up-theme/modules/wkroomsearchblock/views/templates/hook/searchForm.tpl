{*
* Farmhouse theme override for wkroomsearchblock searchForm.tpl
* Field names, ids, classes and JS contract are BYTE-IDENTICAL to the module
* template — only the outer markup + labels are restyled.
*}

<form method="POST" id="search_hotel_block_form">
    {hook h='displaySearchFormFieldsBefore'}
    {block name="search_form_fields_wrapper"}
        <div class="fh-search-grid" id="search_form_fields_wrapper">
            {block name='search_form_location'}
                {if isset($location_enabled) && $location_enabled}
                    <div class="fh-search-field fh-search-field--location">
                        <label for="hotel_location" class="fh-search-field__label">{l s='Where' mod='wkroomsearchblock'}</label>
                        <div class="dropdown">
            ...
            {block name='search_form_hotel'}
                ...
                {else}
                    <div class="fh-search-field fh-search-field--hotel">
                        <label for="id_hotel_button" class="fh-search-field__label">{l s='Property' mod='wkroomsearchblock'}</label>
            ...
            {block name='search_form_dates'}
                {if isset($multiple_dates_input) && $multiple_dates_input}
                    <div class="fh-search-field fh-search-field--dates multi-date" id="daterange_value" data-fh-datepicker>
                        <div class="fh-search-dates">
                            <div class="fh-search-date">
                                <label for="daterange_value_from" class="fh-search-field__label">{l s='Check-in' mod='wkroomsearchblock'}</label>
                                ...
                                <div class="form-control header-rmsearch-input input-date" autocomplete="off" id="daterange_value_from" tabindex="-1"><span>{l s='Check-in' mod='wkroomsearchblock'}</span></div>
                            </div>
                            <div class="fh-search-date">
                                <label for="daterange_value_to" class="fh-search-field__label">{l s='Check-out' mod='wkroomsearchblock'}</label>
                                ...
                                <div class="form-control header-rmsearch-input input-date" autocomplete="off" id="daterange_value_to" tabindex="-1"><span>{l s='Check-out' mod='wkroomsearchblock'}</span></div>
                            </div>
                        </div>
                    </div>
                {else}
                    <div class="fh-search-field fh-search-field--dates" data-fh-datepicker>
                        <label for="daterange_value" class="fh-search-field__label">{l s='Check-in' mod='wkroomsearchblock'} – {l s='Check-out' mod='wkroomsearchblock'}</label>
                        <input type="hidden" id="check_in_time" name="check_in_time" {if isset($search_data['date_from'])}value="{$search_data['date_from']|escape:'htmlall':'UTF-8'}"{/if}>
                        <input type="hidden" id="check_out_time" name="check_out_time" {if isset($search_data['date_to'])}value="{$search_data['date_to']|escape:'htmlall':'UTF-8'}"{/if}>
                ...
                {/if}
                <div class="fh-search-quick">
                    <span class="fh-search-quick__label">{l s='Quick dates' mod='wkroomsearchblock'}:</span>
                    <button type="button" class="fh-chip" data-fh-quick="today">{l s='Today' mod='wkroomsearchblock'}</button>
                    <button type="button" class="fh-chip" data-fh-quick="tomorrow">{l s='Tomorrow' mod='wkroomsearchblock'}</button>
                    <button type="button" class="fh-chip" data-fh-quick="plus2">{l s='+2 days' mod='wkroomsearchblock'}</button>
                    <button type="button" class="fh-chip" data-fh-quick="weekend">{l s='Next weekend' mod='wkroomsearchblock'}</button>
                </div>
            {/block}

            {block name='search_form_occupancy'}
                {if isset($is_occupancy_wise_search) && $is_occupancy_wise_search}
                    <div class="fh-search-field fh-search-field--guests">
                        <label for="guest_occupancy" class="fh-search-field__label">{l s='Guests' mod='wkroomsearchblock'}</label>
                        <div class="dropdown">
            ...
            {block name='search_form_submit'}
                <div class="fh-search-field fh-search-field--submit">
                    <button type="submit" class="btn btn-primary fh-btn fh-btn--primary fh-btn--block" name="search_room_submit" id="search_room_submit">
                        <span class="fh-btn__spinner"></span>
    ...
    {/block}
    {hook h='displaySearchFormFieldsAfter'}
</form>
