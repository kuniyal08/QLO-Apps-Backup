...
{block name='order_opc'}
	...
	<section id="wrapper">
		<div class="container ru-opc-shell">
			<section id="content">
				<div class="ru-checkout-intro">
					<div class="ru-eyebrow">{l s='Secure booking'}</div>
					<h1>{l s='Complete your stay'}</h1>
					<p>{l s='Sign in to confirm guest details, then select a payment option.'}</p>
				</div>
				<nav class="ru-checkout-steps" aria-label="{l s='Checkout progress'}"><span class="is-active">{l s='1. Review'}</span><span>{l s='2. Guest details'}</span><span>{l s='3. Payment'}</span></nav>
				<div class="row">
					{if $PS_CATALOG_MODE}
