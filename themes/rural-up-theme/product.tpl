...
{block name='product'}
	...
	{if $errors|@count == 0}
		...
		{/if}
	<div class="product_wrapper ru-room-detail" itemscope itemtype="http://schema.org/Product">
		{if isset($product) && $product}
			<meta itemprop="url" content="{$link->getProductLink($product)}">
				<div class="primary_block row ru-product-layout">
					<div class="col-xs-12 ru-room-intro">
						<span class="ru-eyebrow">{l s='Stay details'}</span>
						<p>{l s='Choose dates and guests to see your complete stay price.'}</p>
					</div>
				<!-- {if !$content_only}
					<div class="container">
